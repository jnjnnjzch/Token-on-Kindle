    ...
    while [ -f "$ENABLED_FILE" ]; do
        ...
        case "$event" in
            ...
            wakeupFromSuspend*|resuming*)
                # RTC wakeups and manual resumes both get a fresh image. update.sh
                # keeps powerd awake long enough to bring Wi-Fi back up.
                sleep 2
                return 0
                ;;
            *)
                # A timeout while awake naturally falls through once remaining <= 0.
                ;;
        esac
    done
    return 1
