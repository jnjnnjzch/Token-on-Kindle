...
#[tauri::command]
pub(crate) fn get_desktop_state(app: AppHandle) -> DesktopInfo {
    info(&app)
}
...
fn create_menu(app: &AppHandle) -> tauri::Result<Menu<tauri::Wry>> {
    ...
    let status = MenuItem::with_id(
        ...
        format!(
            "Codex {} · DeepSeek {} · 火山 {}",
            snapshot.codex, snapshot.deepseek, snapshot.volcengine
        ),
        false,
        None::<&str>,
    )?;
    ...
    let volcengine =
        MenuItem::with_id(app, "volcengine", "打开火山方舟企业版", true, None::<&str>)?;
    ...
    let copy_browser = MenuItem::with_id(
        app,
        "copy_browser",
        "复制 Kindle 浏览器地址",
        true,
...
pub(crate) fn build_tray(app: &tauri::App) -> tauri::Result<()> {
    ...
    let tray = TrayIconBuilder::with_id("main")
        ...
        .on_menu_event(|app, event| match event.id.as_ref() {
            ...
            "open_browser" => {
                let _ = open_browser_url(app.clone());
            }
            "copy_browser" => {
                let _ = copy_browser_url(app.clone());
            }
            "autostart" => {
    ...
    let _ = app
        .handle()
        .emit_to("main", "desktop-state-changed", info(app.handle()));
...
#[cfg(any(target_os = "android", target_os = "ios"))]
pub(crate) fn build_tray(_app: &tauri::App) -> tauri::Result<()> {
    Ok(())
}
