...
// Native portable updater (Windows): download, checksum, replace current EXE, restart.
...
lib = replaceOrThrow(
  ...
  `#[tauri::command]\nasync fn refresh_sources(app: AppHandle) -> Result<(), String> {\n    reload_sources(&app)\n}\n${updaterCode}\n`,
  'refresh command insertion point'
);
lib = replaceOrThrow(
  lib,
  `            open_source,\n            refresh_sources\n        ])`,
...
write('web/diagnostics.js', String.raw`
const number = value => {
...
function updateSourceCard(source, payload) {
  ...
  if (!hasUsefulData(source, payload)) {
...
write('web/update.js', String.raw`
...
if (versionTarget) versionTarget.textContent = `当前 ${APP_VERSION}`;
if (shouldAutoCheck()) setTimeout(() => checkForUpdates(), 2500);
`);

write('web/index.html', String.raw`
<!doctype html>
<html lang="zh-CN">
<head>
  <meta charset="utf-8">
  ...
  <main class="app-shell">
    <header class="topbar">
      <div class="brand">
        ...
        <div><h1>Token on Kindle</h1><p>Codex 与 DeepSeek 的电子墨水用量看板</p></div>
      </div>
      <div id="service" class="service-pill">正在启动后台服务…</div>
    </header>

    <section class="source-strip" aria-label="数据源状态与控制">
      <button id="open-codex" class="source-card" data-state="idle">
        <span class="source-monogram">C</span>
        <span class="source-copy"><span class="button-kicker">CODEX</span><strong id="codex-connection">需要登录</strong><small id="codex-detail">点击打开登录页面</small></span>
        <span class="source-action">打开</span>
      </button>
      <button id="open-deepseek" class="source-card" data-state="idle">
        <span class="source-monogram">D</span>
        <span class="source-copy"><span class="button-kicker">DEEPSEEK</span><strong id="deepseek-connection">需要登录</strong><small id="deepseek-detail">点击打开登录页面</small></span>
        <span class="source-action">打开</span>
      </button>
    ...
    <section class="workspace">
      <article class="panel preview-panel">
        <div class="panel-heading">
          <div><span class="eyebrow">KINDLE PREVIEW</span><h2>锁屏预览</h2></div>
          <div id="format-note" class="format-note">600 × 800 · 8 位灰度 PNG</div>
        </div>
        <div class="preview-stage"><div class="kindle-frame"><canvas id="dashboard" width="600" height="800"></canvas></div></div>
      </article>

      <aside class="control-column">
        <section class="panel connection-panel">
          <div class="section-title"><div><span class="eyebrow">KINDLE CONNECTION</span><h2>局域网访问</h2></div><span class="section-badge">同一张 PNG</span></div>
          <label class="field-label">Kindle 浏览器地址</label>
          <div class="url-row"><div class="url-box"><code id="browser-url">正在读取局域网地址…</code></div><button id="copy-browser" class="icon-button">复制</button></div>
          <p class="help-text">未越狱 Kindle 直接打开。HTTP 页面只显示与锁屏预览完全相同的图片。</p>
          <details class="secondary-endpoint"><summary>屏保插件图片地址</summary><div class="url-row compact-row"><div class="url-box"><code id="url">正在读取图片地址…</code></div><button id="copy" class="icon-button">复制</button></div></details>
        </section>

        <section class="panel settings-panel">
          <div class="section-title"><div><span class="eyebrow">OUTPUT & SYNC</span><h2>输出与同步</h2></div></div>
          <div class="settings-grid">
            <div class="setting-block"><label class="field-label" for="kindle-profile">Kindle 屏幕型号</label><select id="kindle-profile" class="profile-select" aria-describedby="profile-description"></select><p id="profile-description" class="field-note">Kindle 经典 6 英寸机型</p></div>
            <div class="setting-block"><label class="field-label" for="refresh-minutes">后台刷新间隔</label><div class="interval-control"><input id="refresh-minutes" class="interval-input" type="number" min="1" max="1440" step="1" value="10"><button id="save-refresh" class="save-button">保存</button></div><p id="refresh-value" class="field-note">每 10 分钟刷新</p></div>
          </div>
        </section>

        <section class="panel app-update-panel">
          <div class="section-title"><div><span class="eyebrow">APP UPDATE</span><h2>应用更新</h2></div><span id="installed-version" class="format-note">读取版本中…</span></div>
          <p id="update-status" class="update-status">启动后自动检查 GitHub Releases</p>
          <div class="update-actions"><button id="check-update" class="secondary-action">检查更新</button><button id="install-update" class="install-button" hidden>下载、安装并重启</button></div>
          <p class="help-text">Windows 便携版会校验 SHA-256，退出后替换当前 EXE 并自动重启。应用数据和登录状态不会删除。</p>
        </section>

        <details class="panel debug-panel">
          <summary><span>采集详情</span><small>仅保留排查所需字段</small></summary>
          <div class="status-grid"><article><h3>Codex</h3><pre id="codex-status">尚未采集</pre></article><article><h3>DeepSeek</h3><pre id="deepseek-status">尚未采集</pre></article></div>
        </details>
      </aside>
    </section>
  </main>
...
write('web/styles.css', String.raw`
:root { font-family: Inter, ui-sans-serif, system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", "Microsoft YaHei", sans-serif; color:#171717; background:#ecece8; font-synthesis:none; }
* { box-sizing:border-box; }
body { margin:0; min-width:760px; min-height:100vh; background:linear-gradient(145deg,#f8f8f5 0,#ecece8 42%,#e4e4df 100%); }
button,summary,input,select { font:inherit; } button,summary { cursor:pointer; }
h1,h2,h3,p { margin-top:0; }
.app-shell { width:min(1280px,calc(100% - 34px)); margin:0 auto; padding:22px 0 34px; }
.topbar { display:flex; align-items:center; justify-content:space-between; gap:20px; padding:2px 2px 17px; border-bottom:1px solid #c9c9c2; }
.brand { display:flex; align-items:center; gap:13px; }
.brand-mark { width:38px; height:47px; padding:4px; border:2px solid #171717; border-radius:7px; background:#fff; box-shadow:0 5px 15px rgba(0,0,0,.08); }
.brand-mark span { display:block; width:100%; height:100%; background:linear-gradient(to top,#111 0 58%,#d8d8d2 58% 100%); border-radius:2px; }
h1 { margin-bottom:2px; font-size:27px; line-height:1.1; letter-spacing:-.55px; }
.brand p { margin:0; color:#6a6a64; font-size:13px; }
.service-pill { max-width:430px; padding:8px 13px; border:1px solid #c8c8c1; border-radius:999px; background:rgba(255,255,255,.72); color:#4f4f49; font-size:12px; text-align:center; }
.source-strip { display:grid; grid-template-columns:minmax(240px,1fr) minmax(240px,1fr) 178px; gap:12px; padding:15px 0; }
.source-card,.refresh-button { min-height:74px; border:1px solid #c8c8c1; border-radius:15px; transition:.15s ease; }
.source-card:hover,.refresh-button:hover,.icon-button:hover,.save-button:hover,.secondary-action:hover,.install-button:hover { transform:translateY(-1px); box-shadow:0 9px 20px rgba(0,0,0,.08); }
.source-card { display:grid; grid-template-columns:42px 1fr auto; align-items:center; gap:12px; padding:11px 13px; background:rgba(255,255,255,.88); text-align:left; }
.source-card[data-state="connected"] { border-color:#8e9e90; background:#f7faf6; }
.source-card[data-state="error"] { border-color:#b98f8f; background:#fcf7f7; }
.source-monogram { display:grid; place-items:center; width:40px; height:40px; border-radius:11px; background:#1b1b1b; color:#fff; font-weight:800; }
.source-copy { min-width:0; display:flex; flex-direction:column; gap:2px; }
.source-copy strong { font-size:15px; }
.source-copy small { overflow:hidden; color:#707069; font-size:11px; text-overflow:ellipsis; white-space:nowrap; }
.button-kicker,.eyebrow { color:#777770; font-size:9px; font-weight:800; letter-spacing:1.35px; }
.source-action { color:#52524d; font-size:11px; font-weight:750; }
.refresh-button { display:flex; flex-direction:column; align-items:center; justify-content:center; gap:4px; padding:10px 16px; background:#181818; color:#fff; font-weight:760; }
.refresh-button small { color:#cfcfca; font-size:10px; font-weight:500; }
.refresh-button:disabled { cursor:wait; opacity:.65; }
.workspace { display:grid; grid-template-columns:minmax(510px,1fr) 410px; gap:16px; align-items:start; }
.panel { border:1px solid #d0d0c9; border-radius:18px; background:rgba(255,255,255,.9); box-shadow:0 13px 32px rgba(0,0,0,.05); }
.preview-panel { padding:18px; }
.panel-heading,.section-title { display:flex; align-items:flex-start; justify-content:space-between; gap:14px; }
.panel-heading { align-items:flex-end; margin-bottom:14px; }
.panel-heading h2,.section-title h2 { margin:4px 0 0; font-size:20px; letter-spacing:-.3px; }
.format-note,.section-badge { color:#777770; font-size:11px; }
.section-badge { padding:5px 8px; border-radius:999px; background:#ededE8; font-weight:700; }
.preview-stage { display:grid; place-items:center; min-height:620px; padding:20px; border-radius:14px; background:linear-gradient(135deg,rgba(255,255,255,.54),transparent 47%),#dcdcd6; }
.kindle-frame { padding:10px; border:2px solid #222; border-radius:14px; background:#2b2b2b; box-shadow:0 22px 38px rgba(0,0,0,.18); }
canvas { display:block; width:390px; height:520px; border-radius:3px; background:#fff; }
.control-column { display:grid; gap:13px; }
.connection-panel,.settings-panel,.app-update-panel { padding:17px; }
.field-label { display:block; margin:14px 0 7px; color:#565650; font-size:11px; font-weight:760; }
.url-row { display:grid; grid-template-columns:minmax(0,1fr) 66px; gap:8px; align-items:stretch; }
.url-box { min-width:0; padding:10px 11px; border:1px solid #cecec7; border-radius:10px; background:#f3f3ef; }
.url-box code { display:block; overflow-wrap:anywhere; color:#292929; font-size:11px; line-height:1.45; }
.icon-button,.save-button,.secondary-action,.install-button { border:1px solid #bdbdb6; border-radius:10px; font-weight:750; transition:.14s ease; }
.icon-button { background:#fff; font-size:11px; }
.help-text { margin:11px 0 0; color:#6b6b64; font-size:11px; line-height:1.58; }
.secondary-endpoint { margin-top:12px; padding-top:10px; border-top:1px solid #e2e2dc; }
.secondary-endpoint summary { color:#55554f; font-size:11px; font-weight:750; }
.compact-row { margin-top:9px; }
.settings-grid { display:grid; grid-template-columns:1fr 1fr; gap:13px; margin-top:13px; }
.setting-block { min-width:0; padding:12px; border:1px solid #dfdfd9; border-radius:12px; background:#fafaf8; }
.setting-block .field-label { margin-top:0; }
.profile-select,.interval-input { width:100%; min-width:0; padding:9px 10px; border:1px solid #bdbdb6; border-radius:9px; background:#fff; color:#181818; font-size:11px; font-weight:650; }
.interval-control { display:grid; grid-template-columns:1fr 62px; gap:7px; }
.save-button { background:#1a1a1a; color:#fff; font-size:11px; }
.field-note { margin:7px 0 0; color:#777770; font-size:10px; line-height:1.45; }
.debug-panel { overflow:hidden; }
.debug-panel summary { display:flex; align-items:center; justify-content:space-between; gap:12px; padding:14px 16px; color:#454540; font-size:12px; font-weight:760; list-style:none; }
.debug-panel summary::-webkit-details-marker { display:none; }
.debug-panel summary small { color:#85857e; font-size:10px; font-weight:500; }
.status-grid { display:grid; gap:9px; padding:0 12px 12px; }
.status-grid article { min-width:0; padding:11px; border-radius:10px; background:#f1f1ed; }
.status-grid h3 { margin-bottom:7px; font-size:12px; }
pre { max-height:210px; margin:0; overflow:auto; white-space:pre-wrap; word-break:break-word; color:#50504a; font:10px/1.5 ui-monospace,SFMono-Regular,Consolas,monospace; }
@media (max-width:1040px) { .workspace { grid-template-columns:1fr 370px; } canvas { width:360px; height:480px; } .preview-stage { min-height:580px; } }
@media (max-width:900px) { body { min-width:620px; } .workspace { grid-template-columns:1fr; } .control-column { grid-template-columns:1fr 1fr; } .connection-panel,.debug-panel { grid-column:1 / -1; } }
@media (max-width:680px) { body { min-width:0; } .app-shell { width:calc(100% - 22px); padding-top:14px; } .topbar { align-items:flex-start; flex-direction:column; } .source-strip { grid-template-columns:1fr; } .workspace,.control-column,.settings-grid { grid-template-columns:1fr; } .connection-panel,.debug-panel { grid-column:auto; } canvas { width:min(78vw,390px); height:auto; aspect-ratio:3/4; } .preview-stage { min-height:auto; padding:14px; } }
`);

write('web/update.css', String.raw`
.update-status { min-height:20px; margin:12px 0; color:#5e5e58; font-size:12px; line-height:1.5; }
.update-status[data-state="ok"] { color:#34613d; }
.update-status[data-state="available"] { color:#1d1d1d; font-weight:700; }
.update-status[data-state="error"] { color:#8b3030; }
.update-actions { display:grid; grid-template-columns:128px 1fr; gap:8px; }
.secondary-action,.install-button { min-height:40px; padding:9px 12px; }
.secondary-action { background:#fff; color:#1b1b1b; }
.install-button { background:#181818; color:#fff; border-color:#181818; }
.install-button[hidden] { display:none; }
.secondary-action:disabled,.install-button:disabled { cursor:wait; opacity:.62; }
`);
...
let cargo = read('src-tauri/Cargo.toml').replace(/^version = ".*"/m, 'version = "0.6.0"');
write('src-tauri/Cargo.toml', cargo);
const tauriConfig = JSON.parse(read('src-tauri/tauri.conf.json'));
tauriConfig.version = '0.6.0';
...
if (fs.existsSync('tests/version-target.test.mjs')) {
  write('tests/version-target.test.mjs', read('tests/version-target.test.mjs').replace(/v0\.5\.5/g, 'v0.6.0').replace(/0\.5\.5/g, '0.6.0'));
}

write('tests/diagnostic-summary.test.mjs', String.raw`
...
test('DeepSeek diagnostic summary keeps useful fields and removes noisy raw data', () => {
  const summary = diagnosticSnapshot('deepseek', {
    source: 'deepseek',
    capturedAt: '2026-08-02T08:31:25.126Z',
    ...
    diagnostics: {
      chartCount: 5,
      tooltipCount: 0,
      visibleSummary: { lineCount: 77 },
      parser: { source: 'platform-internal-api', attempts: [{ date: 'x' }], amountDayCount: 31 }
    }
  });
...
write('tests/updater-contract.test.mjs', String.raw`
import test from 'node:test';
import assert from 'node:assert/strict';
import fs from 'node:fs';
...
test('Windows portable updater downloads, verifies, replaces, and restarts', () => {
  ...
  assert.match(native, /Wait-Process/);
  assert.match(native, /Copy-Item/);
  assert.match(native, /Start-Process/);
...
write('tests/source-status-ui.test.mjs', String.raw`
import test from 'node:test';
import assert from 'node:assert/strict';
import fs from 'node:fs';
...
test('source controls expose real connection states instead of fixed login copy', () => {
  assert.match(html, /id="codex-connection"/);
  assert.match(html, /id="deepseek-connection"/);
  assert.match(app, /status\.textContent = '已连接'/);
  assert.match(app, /status\.textContent = '同步失败'/);
  assert.match(app, /status\.textContent = '需要登录'/);
});
`);

write('tests/repository-cleanup-contract.test.mjs', String.raw`
import test from 'node:test';
import assert from 'node:assert/strict';
import fs from 'node:fs';
...
test('repository keeps one reusable workflow and cleans merged branches plus legacy run history', () => {
  assert.deepEqual(workflows, ['pipeline.yml']);
  assert.match(pipeline, /workflow_call:/);
  assert.match(pipeline, /Repository cleanup/);
  assert.match(pipeline, /merge-base --is-ancestor/);
  assert.match(pipeline, /Clean stale branches/);
  assert.match(pipeline, /Windows Portable/);
});
`);

// Remove any accidentally restored duplicate workflow definitions.
for (const path of [
...
let pipeline = execFileSync('git', ['show', 'origin/main:.github/workflows/pipeline.yml'], { encoding: 'utf8' });
pipeline = pipeline.replace('permissions:\n  contents: write', 'permissions:\n  contents: write\n  actions: write');
if (!pipeline.includes('name: Repository cleanup')) {
  pipeline += String.raw`
      ...
      - name: Remove merged work branches
        ...
        run: |
          ...
          gh api --paginate "repos/$GITHUB_REPOSITORY/branches?per_page=100" --jq '.[].name' | while read -r branch; do
            ...
            if git show-ref --verify --quiet "$remote_ref" && git merge-base --is-ancestor "origin/$branch" origin/main; then
...
// The generator is only a branch bootstrapper; do not leave it in the product tree.
fs.rmSync('tools/generate-v0.6.0.mjs');
console.log('Generated v0.6.0 maintenance release sources.');
