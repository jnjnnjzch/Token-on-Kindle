...
test('internal API parser preserves AFP and per-model token categories', () => {
  const parsed = parseVolcengineInternalApiPayloads({
    ...
    modelListBody: { Result: { Data: [
      { RespModelID: 'deepseek-v4-flash' },
      { RespModelID: 'kimi-k2.6' }
    ] } },
    usageBody: { Result: { SeatUsageDetails: [{ UsageDetails: [
      {
      ...
      },
      {
        ...
        UsageResults: [{ Name: 'TotalTokens', MetricItems: [{ Values: [{ Timestamp: 10, Value: 7 }, { Timestamp: 20, Value: 9 }] }] }]
      }
    ] }] } },
    usageRequestBody: { Filter: { StartTime: '2026-08-01', EndTime: '2026-08-07' }, QueryInterval: 'Day' }
  });
...
test('Volcengine production reader is a session API worker without DOM or ECharts fallback', async () => {
  const [reader, built] = await Promise.all([
    readFile(new URL('../web/volcengine-direct-reader.js', import.meta.url), 'utf8'),
  ...
  for (const marker of [
    ...
    assert.ok(reader.includes(marker), `reader missing ${marker}`);
    assert.ok(built.includes(marker), `built extractor missing ${marker}`);
  }
  assert.match(built, /console-internal-api/);
...
test('diagnostic JSON keeps internal API model fields', () => {
  const snapshot = diagnosticSnapshot('volcengine', {
    ...
    unit: 'AFP',
    windows: [],
    models: [{ name: 'kimi-k2.6', totalTokens: 1234, latestTokens: 321, inputTokens: 600, cachedTokens: 300, outputTokens: 334, pointCount: 7 }],
    modelUsage: { source: 'console-internal-api', periodStart: '2026-08-01', periodEnd: '2026-08-07', granularity: 'Day' },
    diagnostics: { primarySource: 'console-internal-api', lifecycle: 'v0.6.2-reload-worker', modelCount: 1 }
  });
